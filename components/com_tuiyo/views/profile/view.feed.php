...
class TuiyoViewProfile extends JView{
	...
	public function showFeed(){
		...
		$uData 	= TuiyoAPI::get('user', $profileID );
		
		if(empty($profileID) || (!$uData) || $uData->id < 1 ){
			JError::raiseError(TUIYO_NOT_FOUND, _('The profile feed either does not exists or you do not have the necessary permissions'));
			return false;
		}
		
		$tData 		= $tModel->getUserTimeline( $uData->id , $tUser->id );
		...
		$document->title =  $uData->name ;
		$document->description = sprintf( _( "Updates from %s / profile"), $uData->name);
		$document->link  =  JRoute::_(TUIYO_INDEX.'&amp;view=profile&amp;do=getFeed&amp;format=feed&amp;pid='.$uData->name );	
		...
		foreach($tData as $stream):	// load individual item creator class
			...
			$item->authorEmail  = "{$item->author} via {$uData->email}";
