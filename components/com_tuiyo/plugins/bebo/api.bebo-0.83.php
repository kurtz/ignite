 ...
 * $bebo->photos_upload($album_id, $caption, $filename, $tempfile); // upload photo
 ...
 * Bugs: http://developer.bebo.com/bugzilla/
 **/
...
class Bebo {
   ...
   * @param string $caption Optional : a caption for this photo
   * @param string $filename : the photo file name
   * @param string $tempfile : the path to the temp file
   */
   public function photos_upload($aid, $caption, $filename, $tempfile) {
    return $this->execute('photos.upload',
      array('aid' => $aid,
            'caption' => $caption,
            'filename' => $filename,
            'tempfile' => $tempfile));
  ...
  /**
   * Tests whether or not the user identified by the session key has granted the 
   * extended permission specified by the ext_perm parameter. Returns true (1)
   * if they have or false (0) if they haven't.
   * @param string ext_perm The name of the extended permission you are checking to see if the user posesses. 
   */
  public function users_hasAppPermission($ext_perm) {
    return $this->execute('users.hasAppPermission', array('ext_perm' => $ext_perm));
  }

  /**
   * Sets the user's Saying to the text provided. It's important to note that the Bebo limit 
   * for status text is 140 characters, not 160. This method requires that the status_update
   * permission be granted in order to succeed. 
   * @param string status The status message you wish to set for the user. The status has a maximum of 160 characters
   * @param string clear Set to true if you wish to clear the status message for the user
   * @param string status_includes_verb Here for compatibility only, Bebo does not prepend anything to your status
   * @param string uid Here for compatibility only, this is ignored, and assumed to be the user whose session key is passed in
   */
  public function users_setStatus($status='', $clear='', $status_includes_verb='', $uid='') {
    return $this->execute('users.setStatus', 
			  array('status' => $status,
				'clear' => $clear,
  ...
  public function score_addToCumulative($score, $uid='', $name='') {
    return $this->execute('score.addToCumulative', array('score' => $score, 'member_id' => $uid, 'name' => $name));
  }

  public function score_deleteCumulative($uid='', $name='') {
    return $this->execute('score.deleteCumulative', array('member_id' => $uid, 'name' => $name));
  }

  public function score_deleteHigh($uid='', $name='') {
    return $this->execute('score.deleteHigh', array('member_id' => $uid, 'name' => $name));
  }

  public function score_getCumulative($uid='', $name='') {
    return $this->execute('score.getCumulative', array('member_id' => $uid, 'name' => $name));
  }
  ...
  public function score_setHigh($score, $uid='', $name='') {
    return $this->execute('score.setHigh', array('score' => $score, 'member_id' => $uid, 'name' => $name));
  }


  public function leaderboard_aroundYou($size=5, $filter='', $type='', $name='') {
    return $this->execute('leaderboard.aroundYou', array('board_type' => $type, 'name' => $name, 'size' => $size, 'filter' => $filter));
  }

  public function leaderboard_delete($type, $name='') {
    return $this->execute('leaderboard.delete', array('board_type' => $type, 'name' => $name));
  }

  public function leaderboard_topScores($size, $type='', $name='') {
    return $this->execute('leaderboard.topScores', array('board_type' => $type, 'name' => $name, 'size' => $size));
  }
  ...
  protected function execute($method, $params) {
    ...
    if ($method == 'photos.upload') {
      ...
      $content[] = 'Content-Disposition: form-data; name="data"; filename="' . $filename . '"' . "\r\n" .
                   ...
                   file_get_contents($params['tempfile']) . "\r\n--" . $boundary;
      $content[] = array_pop($content) . '--';

      $content = implode("\r\n", $content);
         if (function_exists('curl_init')) {
              $header1 = array('User-Agent: Bebo PHP5 Client 0.9 ' . phpversion(),
