...
(function($){
        ...
        return {
            ...
            slideshow : function(albumid){
                ...
                $(this).click(function(e, albumid){
                    ...
                    $('div#slideshowScreenPanel').show().focus();
		                    
                    //Rescale Image
                    var thumbsState = 1,
                    maxHeight   = parseInt(window.innerHeight-105),
                    maxWidth 	 = $('div.slideshowScreenPad').width(),
                    newHeight   = maxHeight,
                    newWidth    = maxWidth,
                    bottomA     = maxHeight-130,
                    bottomB     = bottomA+100,
                    topPosition = $('div.slideshowScreenPad').offset().top,
                    images 	 = [],
                    pointer 	 = 0,  
                    rescaleImage = function(){
                        ...
                        if(imgArea > maxArea){
                            ...
                            var imgProportion = Math.abs(imgArea - maxArea )/maxArea;
                            //Fake Absolute!!
                            //If proportion is greater than 100% divide if not multiple to avoid doubling or negative values
                            newWidth    = imgwidth - ((imgProportion>1)?imgwidth/imgProportion:imgwidth*imgProportion);
                            newHeight   = imgheight - ((imgProportion>1)?imgheight/imgProportion:imgheight*imgProportion);
                        ...
                        $(bgImg).css({
                            ...
                            "top"   : (maxHeight- newHeight-15)/2
                        });
                    };
		             	 
                    //rescaleImage();	
                    $('div.slideshowScreenPad').height( maxHeight );
                    bottomA = maxHeight - 130;
                    bottomB = bottomA+100;
                    $('div.slideShowTools').css({
                        'bottom': '-'+bottomB+'px'
                    });
		                	 
                    $('div.thumbsToggleControl').unbind('click').bind('click',function(e){
                        e.preventDefault();
                        var offsetBottom = $('div.slideShowTools');
                        bottomA = maxHeight - 130;
                        bottomB = bottomA+100;
                        $('div.slideShowTools').css({
                            'bottom': '-'+bottomB+'px'
                    ...
                    $.getJSON('index.php',
                    {
                        ...
                        "do":"slideShow", 
                        "format":"json", 
                        "aid":(!albumid || typeof albumid == 'undefined')? '' : albumid
                    },function(response){
                        ...
                        $("div.previousControl").unbind("click").bind("click",function(){
                            ...
                            $("img#slideshowImageSrc").attr("src" , images[imgpoint].src_original).load(function(){
                                rescaleImage();
                            });
                            $("div.titleControl").find("span#titleSpan").text( images[imgpoint].caption);
                        ...
                        $("img#slideshowImageSrc").attr("src",images[0].src_original).load(function(){
                            rescaleImage();
                        });
                        $("div.titleControl").find("span#titleSpan").text( images[0].caption);
            ...
            viewAlbum : function(){
                $(this).click(function(e){
                    ...
                    function(inResponse){
                        ...
                        {
                            tagName:'div', 
                            className:'infoGroupTitle', 
                            style:'margin-top: 0px', 
                            innerHTML:'<h3 style="padding: 0px">'+album.name+'</h3>'
                        },
                        ...
                        {
                            tagName:'div', 
                            className:'infoGroupTitle slideshowLink', 
...

                            {
                                tagName:'a', 
                                href:'#', 
                                innerHTML:'<h3 style="padding: 0px">'+$.gt.gettext('Album Photos')+'</h3>'
                            }
                            ]
                        },
                        ...
                        $.each(photos, function(i, photo){						
                            ...
                            {
                                tagName:'a', 
                                className:'imageItemWithDescr', 
                                href: photo.src_original, 
                                rel:'facebox', 
                                childNodes:[
    ...
    $(document).ready(function(){
        ...
        $("a#closeSlideshow").click(function(e){
            e.preventDefault();
            $("#slideshowScreenPanel").hide();
        })
        $(".createAlbum").TuiyoPhotosNewAlbum();
        $(".organizeAlbums").TuiyoPhotosOrganize();
        $("ul.publisherTabItems li:first").each(function(i) {
