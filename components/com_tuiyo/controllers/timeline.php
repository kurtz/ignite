...
//      	}
    ...
    public function getStatus() {
        ...
        if ($userID <> $user->id)
            JError::raiseError(TUIYO_BAD_REQUEST, "Invalid user ID");
    ...
    public function addTimelineRow($type= "status") {
        ...
        $postData['ptext'] = substr($postData['ptext'], 0, (int)$limit );
        ...
        $resp["data"] = array(
            "status" => TuiyoSmileys::parse( $tData->data ),
            "statusID" => $tData->ID,
            "source" => $tData->source,
    ...
    public function getComments() {
        ...
        if ($userID <> $user->id)
            JError::raiseError(TUIYO_BAD_REQUEST, _("Invalid user ID"));
        if (!JRequest::checkToken("post"))
            JError::raiseError(TUIYO_BAD_REQUEST, _("Invalid token"));
    }
    ...
    public function getPublicTimeline($filterType = null) {
        ...
        if ($method !== 'get')
            JError::raiseError(TUIYO_BAD_REQUEST, _("Invalid request. Method accepts only get request"));
