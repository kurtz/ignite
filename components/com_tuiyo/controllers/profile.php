...
class TuiyoControllerProfile extends JController {
    ...
    public function viewStatus() {
        ...
        if (!empty($statusID) || (int) $statusID > 0) {
            $document->addMetaTag("sid", (int) $statusID);
        }
        return $this->view();
    }
    ...
    public function view($tpl = null) {
        ...
        $document->addMetaTag("pid", $thatuser->id);
    ...
    public function saveParams() {
        ...
        if (!isset($key) || empty($key))
            trigger_error("INVALID PARAM KEY", E_USER_ERROR);
        if (!JRequest::checkToken() || $user->id <> (int) $post["userid"])
            trigger_error("INVALID USER TOKEN", E_USER_ERROR);
